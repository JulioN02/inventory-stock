...
export function setRefreshCookie(res: Response, token: string, config: AppConfig): void {
  res.cookie(REFRESH_COOKIE_NAME, token, {
    ...
    maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days (AUTH-2)
  })
}

export function clearRefreshCookie(res: Response, config: AppConfig): void {
  // Note: no `signed` flag here — clearing must produce an empty value,
  // not a signed empty string. Same path/attrs so the browser matches it.
