...
export function createAuthController(deps: { db: Pool; config: AppConfig }): AuthController {
  ...
  async function login(req: Request, res: Response): Promise<void> {
    const result = await authService.login(db, req.body as LoginInput, config.jwtSecret)
    setRefreshCookie(res, result.refreshToken, config)
    res.json({ user: result.user, accessToken: result.accessToken })
  }

  async function refresh(req: Request, res: Response): Promise<void> {
    const result = await authService.refresh(db, readRefreshCookie(req), config.jwtSecret)
    setRefreshCookie(res, result.refreshToken, config)
    res.json({ user: result.user, accessToken: result.accessToken })
  }

  async function logout(req: Request, res: Response): Promise<void> {
    await authService.logout(db, readRefreshCookie(req))
