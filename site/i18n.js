...
const I18N = {
  es: {
    ...
    'nav.fundamentals': 'Fundamentos aplicados',
    ...
    'features.card2.body': 'Invariantes de stock bajo concurrencia; imposible vender stock inexistente.',
    'features.card3.title': 'Idempotencia',
    'features.card3.body': 'Reintentos seguros sin efectos duplicados mediante claves de idempotencia.',
    'features.card4.title': 'Auditoría',
    'features.card4.body': 'Quién, cuándo y qué cambió, de forma inmutable y append-only.',
    ...
    // ── fundamentals (P4) ────────────────────────────────────────────────
    'fundamentals.title': 'Fundamentos aplicados',
    'fundamentals.lead': 'Los principios que sostienen la consistencia del dominio.',
    ...
    'fundamentals.f1.title': 'Consistencia bajo concurrencia',
    'fundamentals.f1.body': 'El advisory lock por producto y almacén impide vender stock inexistente cuando hay operaciones simultáneas.',
    'fundamentals.f2.title': 'Idempotencia',
    'fundamentals.f2.body': 'La clave de idempotencia y el hash del request detectan reintentos y evitan efectos duplicados.',
    'fundamentals.f3.title': 'Auditoría inmutable',
    'fundamentals.f3.body': 'El trigger de base de datos rechaza cualquier mutación del libro mayor y del log de auditoría.',
    ...
    // ── repo (P10b) ──────────────────────────────────────────────────────
    'repo.title': 'Obtener el código',
    'repo.lead': 'El proyecto vive en GitHub y hay dos formas de llevártelo.',
    'repo.clone.title': 'Git clone',
    'repo.clone.body': 'Con Git instalado, copia el repositorio con SSH o HTTPS:',
    'repo.clone.command': '# SSH\ngit clone git@github.com:JulioN02/inventory-stock.git\n# HTTPS (alternativa)\ngit clone https://github.com/JulioN02/inventory-stock.git',
    'repo.zip.title': 'Descargar ZIP',
    'repo.zip.body': 'Sin Git: abre la página del repositorio, pulsa el botón verde <strong>Code</strong> y elige <strong>Download ZIP</strong>.',
    'repo.contents.title': 'Qué obtienes',
    'repo.contents.items': '<li><code>backend/</code> — API Express 5 + TypeScript, con tests</li><li><code>frontend/</code> — SPA React 19 + Vite</li><li><code>docker-compose.yml</code> — PostgreSQL 16 lista para levantar</li><li>npm workspaces — un solo comando instala todo</li>',
    'repo.url': 'Repositorio: <a href="https://github.com/JulioN02/inventory-stock" target="_blank" rel="noopener">github.com/JulioN02/inventory-stock</a>',
    'repo.link': 'Abrir el repositorio',

    // ── guide (P10c) ─────────────────────────────────────────────────────
    'guide.title': 'Guía de uso',
    'guide.lead': 'Cómo usar cada módulo, explicado sin tecnicismos.',
    'guide.whatIs': 'Qué es',
    'guide.howToUse': 'Cómo usarlo',
    'guide.auth.title': 'Auth / Roles',
    'guide.auth.what': 'El control de acceso con roles. Un administrador da de alta a los usuarios y cada rol define lo que se puede hacer.',
    'guide.auth.steps': '<li>Entra como <code>admin</code> con la contraseña <code>ADMIN_PASSWORD</code> de <code>backend/.env</code>.</li><li>Crea usuarios: solo el admin puede registrar; al hacerlo, elige el rol del nuevo usuario.</li><li>Roles: <strong>admin</strong> (control total), <strong>operator</strong> (gestiona catálogo y movimientos), <strong>viewer</strong> (solo lectura) y <strong>auditor</strong> (solo lectura + log de auditoría).</li>',
    'guide.products.title': 'Products (Catálogo)',
    'guide.products.what': 'El catálogo de productos que se compran, venden o almacenan.',
    'guide.products.steps': '<li>Crea un producto con nombre, unidad y precio; el SKU se pasa solo a mayúsculas.</li><li>Busca por texto, por nombre o SKU.</li><li>Desactiva en lugar de borrar: así se conserva el historial de movimientos.</li>',
    'guide.warehouses.title': 'Warehouses (Almacenes)',
    'guide.warehouses.what': 'Las ubicaciones físicas donde se guarda el stock.',
    'guide.warehouses.steps': '<li>Crea un almacén con nombre y código.</li><li>Desactívalo si deja de usarse; tampoco se borra.</li>',
    'guide.movements.title': 'Movements (Movimientos)',
    'guide.movements.what': 'El libro mayor inmutable: cada movimiento suma o resta stock y nunca se edita ni se borra.',
    'guide.movements.steps': '<li><strong>Recepción o venta:</strong> elige producto y almacén, escribe la cantidad y guarda. La recepción suma stock (+) y la venta lo resta (−).</li><li><strong>Traslado:</strong> mueve stock entre almacenes: sale del origen y entra en el destino en una sola operación atómica. No se puede trasladar al mismo almacén.</li><li><strong>Ajuste:</strong> la única vía de corrección (merma, sobrante, error de conteo o daño); la razón es obligatoria.</li><li><strong>Leer el libro mayor:</strong> filtra por tipo, producto o almacén. Las columnas <em>Stock inicial</em> y <em>Stock final</em> muestran el saldo acumulado en cada movimiento.</li>',
    'guide.stock.title': 'Stock (Dashboard)',
    'guide.stock.what': 'El estado derivado por producto y almacén. No se guarda en ninguna tabla: se calcula desde los movimientos.',
    'guide.stock.steps': '<li>Filtra por almacén para ver el stock de una ubicación concreta.</li><li>Stock bajo: el umbral por defecto es 5; por debajo de ese valor, el producto aparece como bajo stock.</li>',
    'guide.audit.title': 'Audit (Auditoría)',
    'guide.audit.what': 'Un registro de solo lectura: quién hizo qué y cuándo (logins, cambios de productos, movimientos y almacenes). Es append-only: no se puede modificar ni borrar.',
    'guide.audit.steps': '<li><strong>Tipo de actor:</strong> <code>user</code> (persona), <code>system</code> (proceso automático) o <code>anonymous</code> (sin sesión).</li><li><strong>Acción y entidad:</strong> el evento registrado y el objeto afectado.</li><li><strong>Payload:</strong> el detalle del cambio en JSON, ampliable.</li><li><strong>IP y user agent:</strong> el origen de la solicitud.</li>',
  ...
  en: {
    ...
    'nav.fundamentals': 'Applied fundamentals',
    ...
    'features.card2.body': 'Stock invariants under concurrency; overselling is impossible.',
    'features.card3.title': 'Idempotency',
    'features.card3.body': 'Safe retries with no duplicate effects via idempotency keys.',
    'features.card4.title': 'Audit',
    'features.card4.body': 'Who, when, and what changed — immutably and append-only.',
    'features.card5.title': 'SQL',
    ...
    // ── fundamentals (P4) ────────────────────────────────────────────────
    'fundamentals.title': 'Applied fundamentals',
    'fundamentals.lead': 'The principles that uphold domain consistency.',
    ...
    'fundamentals.f1.title': 'Consistency under concurrency',
    'fundamentals.f1.body': 'The per product-and-warehouse advisory lock prevents selling stock that does not exist when operations run simultaneously.',
    'fundamentals.f2.title': 'Idempotency',
    'fundamentals.f2.body': 'The idempotency key and the request hash detect retries and prevent duplicate effects.',
    'fundamentals.f3.title': 'Immutable audit trail',
    'fundamentals.f3.body': 'The database trigger rejects any mutation of the ledger and the audit log.',
    ...
    // ── repo (P10b) ──────────────────────────────────────────────────────
    'repo.title': 'Get the code',
    'repo.lead': 'The project lives on GitHub and there are two ways to get it.',
    'repo.clone.title': 'Git clone',
    'repo.clone.body': 'With Git installed, clone the repository via SSH or HTTPS:',
    'repo.clone.command': '# SSH\ngit clone git@github.com:JulioN02/inventory-stock.git\n# HTTPS (alternative)\ngit clone https://github.com/JulioN02/inventory-stock.git',
    'repo.zip.title': 'Download ZIP',
    'repo.zip.body': 'No Git: open the repository page, click the green <strong>Code</strong> button, and choose <strong>Download ZIP</strong>.',
    'repo.contents.title': 'What you get',
    'repo.contents.items': '<li><code>backend/</code> — Express 5 + TypeScript API, with tests</li><li><code>frontend/</code> — React 19 + Vite SPA</li><li><code>docker-compose.yml</code> — PostgreSQL 16 ready to start</li><li>npm workspaces — a single command installs everything</li>',
    'repo.url': 'Repository: <a href="https://github.com/JulioN02/inventory-stock" target="_blank" rel="noopener">github.com/JulioN02/inventory-stock</a>',
    'repo.link': 'Open the repository',

    // ── guide (P10c) ─────────────────────────────────────────────────────
    'guide.title': 'Usage guide',
    'guide.lead': 'How to use each module, explained in plain language.',
    'guide.whatIs': 'What it is',
    'guide.howToUse': 'How to use it',
    'guide.auth.title': 'Auth / Roles',
    'guide.auth.what': 'Role-based access control. An administrator registers users and each role defines what can be done.',
    'guide.auth.steps': '<li>Log in as <code>admin</code> with the <code>ADMIN_PASSWORD</code> from <code>backend/.env</code>.</li><li>Register users: only the admin can; when creating one, choose the new user\'s role.</li><li>Roles: <strong>admin</strong> (full control), <strong>operator</strong> (manages catalog and movements), <strong>viewer</strong> (read-only) and <strong>auditor</strong> (read-only + audit log).</li>',
    'guide.products.title': 'Products (Catalog)',
    'guide.products.what': 'The catalog of products that are bought, sold, or stored.',
    'guide.products.steps': '<li>Create a product with name, unit, and price; the SKU is automatically uppercased.</li><li>Search by text, by name or SKU.</li><li>Deactivate instead of deleting: the movement history is preserved.</li>',
    'guide.warehouses.title': 'Warehouses',
    'guide.warehouses.what': 'The physical locations where stock is kept.',
    'guide.warehouses.steps': '<li>Create a warehouse with a name and a code.</li><li>Deactivate it if it is no longer used; it is not deleted either.</li>',
    'guide.movements.title': 'Movements',
    'guide.movements.what': 'The immutable ledger: each movement adds or removes stock and is never edited or deleted.',
    'guide.movements.steps': '<li><strong>Receiving or sale:</strong> pick a product and a warehouse, type the quantity, and save. Receiving adds stock (+) and sale removes it (−).</li><li><strong>Transfer:</strong> moves stock between warehouses: it leaves the source and enters the destination in a single atomic operation. You cannot transfer to the same warehouse.</li><li><strong>Adjustment:</strong> the only way to correct (shrinkage, surplus, count error, or damage); the reason is mandatory.</li><li><strong>Reading the ledger:</strong> filter by type, product, or warehouse. The <em>Opening stock</em> and <em>Closing stock</em> columns show the running balance of each movement.</li>',
    'guide.stock.title': 'Stock (Dashboard)',
    'guide.stock.what': 'The derived state per product and warehouse. It is not stored in any table: it is computed from the movements.',
    'guide.stock.steps': '<li>Filter by warehouse to see the stock of a specific location.</li><li>Low stock: the default threshold is 5; below that value, the product is shown as low stock.</li>',
    'guide.audit.title': 'Audit',
    'guide.audit.what': 'A read-only record of who did what and when (logins, product, movement, and warehouse changes). It is append-only: it cannot be modified or deleted.',
    'guide.audit.steps': '<li><strong>Actor type:</strong> <code>user</code> (person), <code>system</code> (automatic process), or <code>anonymous</code> (no session).</li><li><strong>Action and entity:</strong> the recorded event and the affected object.</li><li><strong>Payload:</strong> the change details as JSON, expandable.</li><li><strong>IP and user agent:</strong> the origin of the request.</li>',
